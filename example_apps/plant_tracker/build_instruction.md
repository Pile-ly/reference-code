...
table (manual: that URL with `.md` appended). Both tables are owner-only —
...
GET the alias **with the owner's JWT and the `.md` suffix** and read the 302's
...
- **Signed out** at the app URL: the platform serves its own tiny
  ...
  shows after an in-app sign-out). `GET <app url>/index.md` without a
...
- **A different signed-in account**: they can even mint a token naming
  ...
  table lists, writes, blob downloads, the `.md` manual, the alias.
  Indistinguishable from an app that doesn't exist. That is the contract
  working, not a bug.
- The owner's agent reads `<app url>/index.md` with a token minted for

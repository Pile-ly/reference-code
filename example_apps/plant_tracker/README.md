...
├── spa_mock/             API-free local UX/test mock; never deploy this
...
## Local mock vs. deployable SPA

[`spa/`](./spa/) is the deployable private Pilely application. It intentionally
uses the managed data and blob services described above. [`spa_mock/`](./spa_mock/)
is an isolated in-memory UX companion: it has a visible signed-out gate and an
owner-only session simulation, but makes no network or platform calls. Use the
mock for local interaction review and its fixture-resetting tests; use `spa/`
when following the real deployment instructions.

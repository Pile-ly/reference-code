...
| I need a real backend | [`managed_server/`](./managed_server) | none — it *is* the backend | managed Rust server |

Access at a glance — the same four data-app patterns, by who can do what
(`managed_server` isn't a `simple_db` app, so it has no row here; see its
own section below):
...
backend, nothing to keep online. Four
...
## `managed_server/` — a real backend, not a static SPA

**Pattern.** The only project here that isn't a bundle talking to managed
services: a self-contained Rust/axum binary the platform builds through
its own CodeBuild job and runs on its own EC2 box, reached at the app's
host the same way a static SPA is.

**Infrastructure.** A managed server, not `simple_db`/`simple_blob`/
`simple_group` — there is no database, no bundle, no `build_instruction.md`
sequence. The app is built from a source zip, launched on a `t4g.nano`
under systemd, and the platform forwards HTTP straight to the box.

**Access model.** N/A — this project has no `simple_db` tables, so the
`read_group`/`anon_read`/`write_group` shape the four apps above use does
not apply. Identity instead arrives as three forwarded headers
(`X-Pile-Requester-User-Id`/`-Handle`/`-App-Id`) the app trusts directly;
see the project's own README for the full contract.
...
and times out at the edge), answer `GET /healthz`, trust the three
...
sequence — register, build, launch, flip the row.

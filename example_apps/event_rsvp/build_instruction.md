...
of record (GET any route with `.md` appended for its manual), so read the
...
else. Mint one token for THIS app and send that same token to every
`simple_*` service you call — see
[login § Using the token](https://pilely.app/skill/account_management/login)
for the mint call and the check that catches getting it backwards.

```bash
curl -X POST https://pilely.app/~/mint/app_id_token \
  -H "Authorization: Bearer <login-token>" -H "content-type: application/json" \
  -d '{"target": "<this app pile_id>"}'
```
...
- `app_type: "frontend"` — this app has no backend; the bundle's
...
3. **The club itself** — also in `spa/src/config.ts`: `HOST.name`,
   ...
   (a frontend app's `index.md` is served for `GET /index.md`).
...
table (manual: that URL with `.md` appended). The two tables are mirror
...
markdown served for `GET /index.md` — carries the **data surface** the
...
bundle is also the edit path later (frontend app: no backend to restart).
...
- `GET <app url>/index.md` returns the bundle's `index.md`.

...
(GET any route with `.md` appended for its manual), so read the manual at
...
3. **The business itself** — `spa/src/config.ts` is the ONE content file:
   ...
   description for AI agents (a frontend app's `index.md` is served for
   `GET /index.md`). The class ids there feed an inquiry's `class`
...
that URL with `.md` appended). One table is the whole database:
...
- `GET <app url>/index.md` returns the bundle's `index.md` — the agent
  surface.

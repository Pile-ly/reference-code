...
describe("index.html theme + agent conformance (standards/spa §7a/§10)", () => {
  ...
  it("carries the .md hint comment for cold agents", () => {
    expect(html).toContain("append .md");
  });
...
describe("public/index.md (standards/spa §7, static apps)", () => {
  it("exists and ends with the platform-manual footer", () => {
    ...
    expect(indexMd).toContain("https://pilely.app/skill.md");
  });
});

// §7c: an app that keeps data owes its reader the DATA SURFACE. Here that is
// what makes the inbox usable at all — a sender has no other way to learn
// the four field names, and a wrong one is silently dropped.
describe("public/index.md data surface (standards/spa §7c)", () => {
  ...
  it("names the table and every column a sender must fill", () => {
    expect(indexMd).toContain("`inquiries`");
    for (const column of ["email", "question", "phone", "class"]) {
  ...
  it("shows the literal write and owner-read calls", () => {
  ...
  it("states that a submitter cannot read the inbox back", () => {
    expect(indexMd).toContain("404");
  });
});

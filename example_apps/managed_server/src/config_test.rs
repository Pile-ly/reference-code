...
fn clear_all() {
    unsafe {
        ...
        env::remove_var("PILELY_BACKEND_TOKEN");
    }
}
...
#[test]
fn backend_token_is_none_when_unset() {
    let _guard = lock_env();
    clear_all();
    unsafe {
        env::set_var("PORT", "8080");
...
#[test]
fn backend_token_is_read_when_present() {
    let _guard = lock_env();
    clear_all();
    unsafe {
        env::set_var("PORT", "8080");
        env::set_var("PILELY_BACKEND_TOKEN", "a.b.c");
    }
    let config = Config::from_env().unwrap();
    assert_eq!(config.backend_token, Some("a.b.c".to_string()));
    clear_all();
}

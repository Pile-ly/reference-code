...
# The app is reached over direct HTTP, so launch.json carries no forwarding
# key and no "mount_prefix" — simple-server-boot refuses a launch.json
# carrying either (LAUNCH_JSON_HAS_TUNNEL_BLOCK) rather than ignoring it.

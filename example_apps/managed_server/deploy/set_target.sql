...
-- 1. Point the row at 'managed_http' explicitly. `register_pilely_pile`
--    already defaults new rows to 'managed_http', so this is a no-op for a
--    freshly registered app; it only matters if the row was ever flipped
--    away from 'managed_http'.

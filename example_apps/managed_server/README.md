...
server starts from.** Unlike every other project in this family, it is not a
...
- **One secret does live on the box: `PILELY_BACKEND_TOKEN`.** The
  ...
  in `launch.json`, the same way `SIMPLE_SERVER_ID` arrives. It identifies
  THIS server of THIS app, acting as the owner through the app, at the
  platform's `simple-*` hosts — never a user, and never anything this app
  reaches on its own initiative. It names no credential this app's own
...
| `GET /platform` | this server's own `PILELY_BACKEND_TOKEN` spent at `simple-db.pilely.app/apps/list`; answers `{status, body}` from that call verbatim, or `503 {"error":"no_backend_token"}` when the variable is unset |
...
says a backend app's `GET` routes answer a markdown manual (the `.md`-suffix
convention) and never JSON. This app does **not** follow that: every `GET`
above answers plain text or JSON, always, suffix or not. That is a safe
simplification here, not an oversight — the platform only strips a `.md`
suffix upstream and forwards the bare path with `Accept: text/markdown` set;
it never requires or renders markdown on the app's behalf, so this backend
is free to ignore that header and answer its own plain text or JSON on that
same hop regardless. A copier building a managed app that wants the dual
markdown/JSON surface is free to add it; this reference project keeps the
smallest thing that satisfies the platform mechanically.

...
├── spa_mock/             interactive local/demo sibling: no platform or API
...
`spa/` is the deployable reference implementation. It talks to Pilely's
managed database and is the project to configure and ship using
`build_instruction.md`. Do not substitute the mock for it in production.

`spa_mock/` is an API-free interactive demonstration for the public example
gallery. It has role switching and mock login, starts from resettable
in-memory fixtures, and deliberately makes no platform calls or use of real
credentials. Its owner editor keeps drafts in a separate private collection
from published posts. Run it locally with `cd spa_mock && npm ci && npm run
dev`; use **Reset demo** (or reload) to restore the fixtures.
...
```bash
cd spa_mock
VITE_BASE=/reference-code/blog/ npm run build
```

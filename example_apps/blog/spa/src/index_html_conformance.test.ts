...
// §7c: an app that keeps data owes its reader the DATA SURFACE — the manual
// is the only place an agent can learn the tables, and a manual that loses
// it leaves every reader guessing.
describe("public/index.md data surface (standards/spa §7c)", () => {
  ...
  it("names every table the app uses", () => {
    for (const table of ["posts", "drafts", "comments", "likes"]) {
  ...
  it("states what a refusal looks like", () => {

...
/** Deterministic 0–3 tint for an event with no cover photo — the same id
 *  always gets the same gradient, so the page doesn't reshuffle colors on
 *  every render (see `--cover-N` in globals.css). */

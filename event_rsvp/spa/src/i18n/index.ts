...
// from `main.tsx` before the app renders. Every user-facing FUNCTIONAL
// string (form labels, buttons, errors, aria) lives in
// `locales/<code>.json`; components read it through `useTranslation()`.
// Marketing content is a different axis — it lives in src/config.ts (the
// one-file rebrand rule) rather than here.
//
...
void i18next
  ...
  .init({
    ...
    detection: {
      ...
      caches: ["localStorage"],
      lookupLocalStorage: "event_rsvp_lang",
    },
  });

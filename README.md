...
- **[`example_apps/managed_server/`](./example_apps/managed_server)** — a
  ...
  runs on its own box, for apps that need actual server-side code instead
  of a UI over the managed database.
...
`README.md` listing the placeholders to fill. Every app except
`managed_server/`, which has no database or bundle to declare: its own
`README.md` and `deploy/` are the whole sequence instead.
...
- **`@pilely/simple-limiter`** — wraps owner-defined rate-limit policies
  ([simple_limiter](https://pilely.app/skill/app_management/simple_limiter)).

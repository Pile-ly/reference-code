...
// the npm registry -- confirmed in local/STOA/agent/dev_surfaces.md.
// A real, unmodified `npm install` against a generated project's
// "@pilely/core": "^0.2.0" would 404. TEST-ONLY infrastructure: rewrite a
...
for (const signIn of [true, false]) {
  test(`(c) full --install build/typecheck/test cycle (${signIn ? "--sign-in" : "--no-sign-in"})`, () => {
    ...
    try {
      ...
      assert.equal(pkgBefore.dependencies["@pilely/core"], "^0.2.0");
      assert.equal(pkgBefore.dependencies["@pilely/simple-db"], "^0.2.0");

...
// Every service package's own current version range. All four track
// @pilely/core's peer range together (0.2.0): simple-db and simple-email
// needed their own 0.2.0 for a requireAppId() guard once core's appId()
// accessor started returning `string | null`; simple-blob and simple-group
// never call appId() themselves, but a peer range stuck at ^0.1.0 would
// make `npm install` unsatisfiable next to core's own ^0.2.0 pin below, so
// they moved too — declaring the compatible range is the point of the
// bump, not a source change.

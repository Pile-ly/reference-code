...
export interface PilelyClient {
  ...
  /** True on an app host (`<label>.pilely.app` / custom domain); false on the
   *  apex AND on loopback, so local dev takes the meta-tag path. */
  isAppOrigin(): boolean;
  apexOrigin(): string;
  /** Where the browser PAGES live — login, mint-authorize, logout. API calls never use this. */
  authOrigin(): string;
  ...
  appId(): string | null;
  /** Start the sign-in dance. Navigates away.
   *
   *  The argument is a mint TARGET, not an app id: the runtime defaults to
   *  `location.host` on an app origin and falls back to the declared
   *  `<meta name="pilely-app">` id only off one. Pass a target to override. */
...
/** The five managed services this scope has typed clients for. */
export type PilelyService =
  | "simple-db"
  | "simple-blob"
  | "simple-group"
  | "simple-email"

...
/**
 * Walks a paged `simple_*` listing route to the end. A `null` `nextCursor` is
 * the loop terminator. Note the services derive it from `rows.len() == limit`,
 * NOT from a fetch-one-extra check: a page that happens to be exactly full
 * still hands back a cursor, so the walk ends on one extra round trip that
 * returns nothing. Correct results, one wasted call. This owns the loop only,
 * not a cursor type: the services use at least five different cursor shapes,
 * so a single cursor type would not fit them.
 *

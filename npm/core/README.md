...
- `serviceOrigin(service)` — derives a managed service's origin from the
  ...
  instance.
- `appId()` — the app id: the `<meta name="pilely-app">` tag when present,
  else the current token's `pile_id` claim once `ready()` has resolved,
  else `null`. `null` is a legitimate answer (no tag, `ready()` not yet
...
- `assertPilelyRuntime()` — a development-only check, never a module side
  ...
  confirm `window.pilely` is present, and — only when a
  `<meta name="pilely-app">` tag exists at all (the tag is optional) — that
  it sits **above** the `client.js` script tag, a real, silent footgun when
  reversed, because `client.js` reads that tag at parse time when present.
...
The ordering `assertPilelyRuntime()` enforces when a `pilely-app` meta tag
is present — that it sits above the `client.js` script tag — is not written
into the platform's own SPA build standard. This assertion is presently the
only enforcement of that rule.

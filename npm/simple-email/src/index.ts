...
export async function send(input: SendInput): Promise<{ send_id: string; recipient_count: number }> {
  const body =
    "template_id" in input
      ? { app_id: requireAppId(), to: input.to, template_id: input.template_id, variables: input.variables }
      : { app_id: requireAppId(), to: input.to, subject: input.subject, html: input.html };
...
export async function createAccount(sendGroup?: string | null): Promise<Account> {
  const body: { app_id: string; send_group?: string | null } = { app_id: requireAppId() };

/** Every `code` the simple_limiter service can put in a `{ok:false, code, reason}`
 *  refusal. Exhaustive as of the 2026-09 review, checked against the service's
 *  `response_json.rs`.
 *
 *  Note this service says `internal_error`, not the `internal` its four
 *  siblings use. That asymmetry is real — do not "correct" it in a matcher. */
export type SimpleLimiterErrorCode =
  | "unauthenticated"

...
`SimpleLimiterErrorCode`: `unsupported_host` / `unsupported_path` (400, the
target is not one this service will police), `target_not_owned` (400 — note
it is a 400, not the 403 its name suggests),
`policy_limit_reached` / `host_policy_limit_reached` (409, the per-owner and
the shared per-host caps), `already_disabled` (409), `not_found` (404),
`bad_request` (400), `unauthenticated` (401), `bad_forwarded_identity` (401),
`rate_limited` (429), and `internal_error` (500).

That last one is spelled `internal_error` here and plain `internal` on the
other four services. The asymmetry is real; do not normalize it away in a
matcher.
